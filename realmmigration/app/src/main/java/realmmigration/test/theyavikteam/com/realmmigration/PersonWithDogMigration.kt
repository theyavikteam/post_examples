...
class PersonWithDogMigration: RealmMigration{
    override fun migrate(realm: DynamicRealm, oldVersion: Long, newVersion: Long) {
        ...
        if (DatabaseConstants.THIRD_VERSION == oldVersion){
            realmSchema.get(Person::class.java.simpleName)?.addRealmObjectField("pet", realmSchema.get(Dog::class.java.simpleName))
            oldVersion.inc()
        }
    }
}